...
describe("TeacherMaxSpanPerDay", () => {
  ...
  it("should account for activity duration in span calculation", () => {
    const longActivity = new Activity("a4", "Long Math Lecture", subject, 180); 
  ...
  it("should always be satisfied when constraint is inactive", () => {
    ...
    assignment.assignActivity(activity1, { day: 0, hour: 8, minute: 0 }, room.id);
    assignment.assignActivity(activity2, { day: 0, hour: 16, minute: 0 }, room.id); 
    expect(constraint.isSatisfied(assignment)).toBe(true);
  });
  ...
  it("should ignore activities for other teachers", () => {
    ...
    assignment.assignActivity(otherActivity2, { day: 0, hour: 16, minute: 0 }, room.id); 
    expect(constraint.isSatisfied(assignment)).toBe(true);
  });
});
