...
import { TeacherMinDaysPerWeek } from "./TeacherMinDaysPerWeek";

describe("TeacherMinDaysPerWeek", () => {
  const DAYS_COUNT = 5;
  ...
  let subject: Subject;
  let teacher: Teacher;
  let constraint: TeacherMinDaysPerWeek;
  let room: Room;
  ...
  beforeEach(() => {
    ...
    constraint = new TeacherMinDaysPerWeek(teacher, MIN_DAYS);
  });

  it("should not be satisfied when no activities are assigned to the teacher", () => {
    // With no activities, there are 0 working days (less than MIN_DAYS)
